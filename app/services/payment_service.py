        ...
        if iso_raw == IsoCode.APPROVED:
            ...
            if isinstance(challenge_data, dict):
                ...
                if redirect_post_url and creq:
                    ...
                    # The form must POST to CardinalCommerce with name="creq" (lowercase, EMV 3DS 2.x).
                    # We include a full HTML page with:
                    #   - explicit charset + CSP allowing form-action to CardinalCommerce
                    #   - dual submit trigger (DOMContentLoaded + window.onload) for max compat
                    #   - visible manual button as fallback if JS is slow or blocked
                    #   - noscript fallback with a submit button
                    ...
                    payment.threeds_challenge_form = (
                        ...
                        f'  <button class="btn" id="manualBtn" '
                        f'onclick="document.getElementById(\'cform\').submit();">'
                        f'  Continuar manualmente &rarr;'
                        f'  </button>'
                        f'</div>'
                        ...
                        f'    <input type="hidden" name="creq" value="{creq}"/>'
                        f'    <button type="submit" style="margin-top:2rem;padding:.8rem 1.5rem;'
                        f'font-size:1rem;cursor:pointer;">Continuar con tu banco &rarr;</button>'
                        f'  </form>'
                        f'</noscript>'
                        f'<script>'
                        f'(function(){{'
                        f'  var submitted = false;'
                        f'  function doSubmit(){{'
                        f'    if(submitted) return;'
                        f'    submitted = true;'
                        f'    try{{ document.getElementById("cform").submit(); }}'
                        f'    catch(e){{'
                        f'      document.getElementById("manualBtn").style.display="inline-block";'
                        f'      document.getElementById("ring").style.animationPlayState="paused";'
                        f'    }}'
                        f'  }}'
                        ...
                        f'    doSubmit();'
                        f'  }}'
                        f'  window.addEventListener("load", doSubmit);'
                        f'  // Fallback: show manual button after 4s if still not submitted'
