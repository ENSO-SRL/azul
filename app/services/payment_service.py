        ...
        if iso_raw == IsoCode.APPROVED:
            ...
            if isinstance(challenge_data, dict):
                ...
                if redirect_post_url and creq:
                    ...
                    # CSP: no default-src so connect-src is open for the fetch probe.
                    # JS probes Cardinal connectivity (5s timeout) before submitting;
                    # shows a Spanish error with instructions if unreachable.
                    ...
                    payment.threeds_challenge_form = (
                        ...
                        f'connect-src https://authentication.cardinalcommerce.com https://*.cardinalcommerce.com; '
                        ...
                        f'.btn{{padding:.7rem 1.5rem;background:#6c63ff;color:#fff;'
                        f'border:none;border-radius:.6rem;font-size:.95rem;cursor:pointer;'
                        f'font-weight:600;display:none;}}'
                        f'</style>'
                        f'</head>'
                        f'<body>'
                        f'<div class="box">'
                        f'  <div class="ring" id="ring"></div>'
                        f'  <h2 id="ttl">Autenticando con tu banco\u2026</h2>'
                        f'  <p id="sub">Ser\u00e1s redirigido en un momento.<br/>No cierres esta ventana.</p>'
                        f'  <div id="err" style="display:none;background:rgba(244,63,94,.08);'
                        f'border:1px solid rgba(244,63,94,.25);border-radius:.75rem;'
                        f'padding:1.2rem;text-align:left;margin-top:1rem;">'
                        f'    <div style="color:#f43f5e;font-weight:600;margin-bottom:.5rem">'
                        f'\u26a0\ufe0f Sin conexi\u00f3n con el banco</div>'
                        f'    <div style="color:#94a3b8;font-size:.85rem;margin-bottom:.6rem">'
                        f'Tu red no puede alcanzar el servidor de autenticaci\u00f3n.</div>'
                        f'    <ul style="padding-left:1.2rem;font-size:.85rem;'
                        f'color:#cbd5e1;line-height:1.9">'
                        f'      <li>Usa <b>datos m\u00f3viles (4G/5G)</b> en lugar de WiFi</li>'
                        f'      <li>Conecta una <b>VPN</b> y vuelve a intentar</li>'
                        f'      <li>Prueba con otro <b>navegador</b></li>'
                        f'    </ul>'
                        f'    <div id="errd" style="font-size:.7rem;color:#475569;'
                        f'font-family:monospace;margin-top:.6rem;word-break:break-all"></div>'
                        f'  </div>'
                        f'</div>'
                        ...
                        f'<script>'
                        f'(function(){{'
                        f'  var CARDINAL="https://authentication.cardinalcommerce.com";'
                        f'  function showErr(d){{'
                        f'    document.getElementById("ring").style.display="none";'
                        f'    document.getElementById("ttl").textContent="";'
                        f'    document.getElementById("sub").style.display="none";'
                        f'    document.getElementById("err").style.display="block";'
                        f'    if(d)document.getElementById("errd").textContent=d;'
                        f'  }}'
                        f'  function doSubmit(){{'
                        f'    try{{document.getElementById("cform").submit();}}'
                        f'    catch(e){{showErr(String(e));}}'
                        f'  }}'
                        f'  var ac=typeof AbortController!=="undefined"?new AbortController():null;'
                        f'  var tid=setTimeout(function(){{if(ac)ac.abort();}},5000);'
                        f'  var opts={{method:"HEAD",mode:"no-cors"}};'
                        f'  if(ac)opts.signal=ac.signal;'
                        f'  fetch(CARDINAL,opts)'
                        f'    .then(function(){{clearTimeout(tid);doSubmit();}})'
                        f'    .catch(function(e){{'
                        f'      clearTimeout(tid);'
                        f'      var isTO=e&&(e.name==="AbortError"||e.name==="TimeoutError");'
                        f'      showErr(isTO?"ERR_CONNECTION_TIMED_OUT (5s)":String(e));'
                        f'    }});'
                        f'}})();'
                        f'</script>'
                        f'</body></html>'
