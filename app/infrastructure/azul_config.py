...
_SECRET_CREDS = os.getenv("AZUL_SECRET_CREDS", "iamatlas/azul/prod/api-credentials")
_SECRET_CERT  = os.getenv("AZUL_SECRET_CERT",  "iamatlas/azul/prod/cert-pem")
_SECRET_KEY   = os.getenv("AZUL_SECRET_KEY",   "iamatlas/azul/prod/cert-key")
...
def _load_azul_config_cached() -> AzulConfig:
    ...
    if _LOCAL_MODE:
        logger.info("[azul_config] AZUL_LOCAL_MODE=1 — loading from env vars / .env file.")
        return _load_from_env()

    logger.info(
        "[azul_config] Loading from AWS Secrets Manager — "
        "creds=%s cert=%s key=%s",
        _SECRET_CREDS, _SECRET_CERT, _SECRET_KEY,
    )
